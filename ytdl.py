...
if len(sys.argv) > 1:
    ...
    class download_thread(threading.Thread):
        ...
        def run(self):
            ...
            if self.type == 'audio':
                ...
                command = 'ffmpeg -i "{}" {} -c copy "{}".{}'.format(url, 
                        self.add_time_commands(start, end, 'ffmpeg_only'), 
        ...
        def check_timings(self):
            ...
            timings =  list(map(lambda x: int(0 if x == '' else x), [st_min.get(), 
                st_sec.get(), en_min.get(), en_sec.get()]))
